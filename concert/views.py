...
def songs(request):
    du = [{"id":1,"title":"duis faucibus accumsan odio curabitur convallis","lyrics":"Morbi non lectus. Aliquam sit amet diam in magna bibendum imperdiet. Nullam orci pede, venenatis non, sodales sed, tincidunt eu, felis."}]
    songs = {"songs":du}
    return render(request, "songs.html", {"songs": du})
...
def concerts(request):
    if request.user.is_authenticated:
        ...
        return render(request, "concerts.html", context={"concerts": lst_of_concert})
    else:
        return HttpResponseRedirect(request, reverse('login'))
